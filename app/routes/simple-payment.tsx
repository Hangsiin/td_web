...
function SimplePaymentPage(): JSX.Element {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      window.IMP.request_pay({
        ...
        if (response.success) {
          // 결제 성공 시 완료 페이지로 이동
          navigate(`/payment/simple-complete?imp_uid=${response.imp_uid}&merchant_uid=${response.merchant_uid}`);
        } else {
          // 결제 실패
  ...
  const validateForm = () => {
    ...
    return requiredFields.every(field => orderInfo[field as keyof OrderInfo].toString().trim() !== '');
  };
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{ 
        ...
        <div style={{ marginBottom: '20px' }}>
          <label style={{ display: 'block', marginBottom: '5px' }}>결제 금액 (원)</label>
          <input
            type="number"
            name="amount"
      ...
      <SimplePaymentModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
