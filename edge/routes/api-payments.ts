...
function generateSignature(oid: string, price: number) {
  ...
  return {
    signature: crypto.createHmac("sha256", signKey).update(message).digest("hex"),
    timestamp
  };
}
...
app.post("/api/payments/prepare", async (req) => {
  try {
    ...
    if (!merchantUid || !amount) {
      return new Response(
        JSON.stringify({ success: false, message: "필수 파라미터가 누락되었습니다." }),
    ...
    return new Response(
      JSON.stringify({ success: false, message: "결제 준비 중 오류가 발생했습니다." }),
...
app.post("/api/payments/verify", async (req) => {
  try {
    ...
    // 결제 정보 조회
    const { data: responseData } = await axios.get(`https://api.iamport.kr/payments/${imp_uid}`, {
    ...
    if (paymentData.status === 'paid' && paymentData.amount === expectedAmount) {
      // 결제 성공 처리 (DB 업데이트 등)
      return new Response(
        JSON.stringify({ success: true, message: "결제가 성공적으로 완료되었습니다." }),
        { headers: { "Content-Type": "application/json" } }
      );
    } else {
      // 결제 실패 처리
      return new Response(
        JSON.stringify({ success: false, message: "결제 금액이 일치하지 않거나 결제가 완료되지 않았습니다." }),
        { headers: { "Content-Type": "application/json" } }
      );
    }
  } catch (error) {
    ...
    return new Response(
      JSON.stringify({ success: false, message: "결제 검증 중 오류가 발생했습니다." }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
});
